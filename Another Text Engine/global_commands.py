...
def _pickup(player, area, elem):
    ...
    for item in area.getItemList():
        if elem in item.getIdentifierList():
            ...
            print elem.capitalize() + " added to your inventory."
            return
...
def _inspect(player, area, elem):
    ...
    if (elem == "inventory" or elem == "backpack" or elem == "pockets" or
        ...
        if player.getInventory() == []:
            ...
            for i in range(len(inventory) - 1):
                print next(iter(inventory[i].getIdentifierList())) + ", ",

            print next(iter(inventory[len(inventory) - 1].getIdentifierList()))
        return
    ...
    for e in masterList:
        if elem in e.getIdentifierList():
