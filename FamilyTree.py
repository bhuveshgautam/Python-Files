...
class Family(object):
    ...
    def cousin(self, a, b):
        ...
        """
        def parentList(member):
            """
            Takes a member and returns a list of its parents.

            member: string | The string used to access the Member object.
            return: list of string | The list of the names of member's parents
            """
            if member == str(self.root):
                return [str(self.root)]

            return [member] + parentList(str(self.names_to_nodes[member].get_parent()))

        def intersect(a, b):
            """
            Takes two lists and cuts them off one after the point they
            intersect.

            a: list | First list
            b: list | Second list
            return: (list, list) | The two lists after being modified
            """
            for elem in a:
                if elem in b:
                    return (a[:a.index(elem) + 1], b[:b.index(elem) + 1])

            return ([], [])

        aParents = parentList(a)
        bParents = parentList(b)

        aIntersected, bIntersected = intersect(aParents, bParents)

        return (min(len(aIntersected) - 2, len(bIntersected) - 2),
                abs(len(aIntersected) - len(bIntersected)))
